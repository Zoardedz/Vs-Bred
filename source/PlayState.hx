				...
				{	
					...
					if ((daNote.mustPress && daNote.tooLate && !FlxG.save.data.downscroll || daNote.mustPress && daNote.tooLate && FlxG.save.data.downscroll) && daNote.mustPress)
					{
							...
							switch(daNote.noteSpecial)
							{
								...
								case "Bred Note of confoooosion":
									healthDrain();
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					if (note.noteSpecial == 'bred')
						{
							health = -9;
						}
					if (note.noteSpecial == 'Bred note of harming')
						{
							healthDrain();
						}
	...
	function popUp():Void
	{
		...
		if (FlxG.random.bool(amount) && appearScreen)
			//again, wildy for code : )
			{
				...
				randomthing.loadGraphic(Paths.image('daPopUp/popup' + FlxG.random.int(1,4), 'shared'));
...
//var ty:ThankYouWildy = "thank you wildy for da code";
